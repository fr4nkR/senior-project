...
train_transform_b = transforms.Compose([
        transforms.RandomRotation(180),      # rotate +/- 10 degrees
        transforms.RandomHorizontalFlip(),  # reverse 50% of images
        transforms.Resize(224),             # resize shortest side to 224 pixels
        transforms.CenterCrop(224),         # crop longest side to 224 pixels at center
        transforms.ToTensor(),
...
train_transform_c = transforms.Compose([
        transforms.ColorJitter(brightness=100, contrast=10, saturation=2, hue=.5),
        transforms.RandomHorizontalFlip(),  # reverse 50% of images
...
train_data = datasets.ImageFolder(os.path.join(root, 'sample')) #, transform=train_transform)

# test_data = datasets.ImageFolder(os.path.join(root, 'test'), transform=test_transform)

trainset_a, = torch.utils.data.random_split(train_data, [1])
...
trainset_a.dataset.transform = train_transform_b
